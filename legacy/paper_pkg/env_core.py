...
Core environment + execution simulator for the paper experiments.

This file is a trimmed extraction from your original `experiments_atg_leo.py`:
- Keeps only core components required by the NEW paper experiment design:
  * ExperimentConfig (+ RuntimeModelConfig)
  * build_env (actual/predicted)
  * sustainable_dag_plan
  * execute_plan_on_env
  * summarize_runs
- Removes legacy experiment runners (Exp1/Exp2) and any CLI main.

You can keep your original `experiments_atg_leo.py` under legacy/ for reference.
"""
